...
(define (file->calories filename)
  ...
  (for/fold ([acc '(0)]) ([maybe-number input-numbers])
    (if maybe-number (cons (+ (car acc) maybe-number) (cdr acc)) (cons 0 acc))))

(define (top-n-calories calories n)
  (take (sort calories >) n))
(define (sum calories)
  (foldl + 0 calories))
